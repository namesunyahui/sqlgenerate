...
import org.example.sqlgenerate.createSql.mapper.MysqlDataTypeMapper;
 ...
 * MySQL SQL生成策略实现
 */
public class MysqlSqlStrategy implements DatabaseSqlStrategy {
    ...
    public String generateCreateTableSql(CreateFrom createFrom, List<ImportExcel> columns) {
        ...
        for (ImportExcel column : columns) {
            ...
            if (StringUtils.isNotEmpty(column.getRemark())) {
                ...
                sqlBuilder.append(" COMMENT '").append(remark).append("'");
            }
        ...
        // 添加字符集和排序规则
        sqlBuilder.append(" DEFAULT CHARSET=")
                .append(dataTypeMapper.getDefaultCharset())
                .append(" COLLATE=")
                .append(dataTypeMapper.getDefaultCollation());
    ...
    public String generateTableCommentSql(CreateFrom createFrom) {
        ...
        return "ALTER TABLE " + quoteIdentifier(createFrom.getTableName()) +
                " COMMENT '" + comment + "';\n";
    }
    ...
    public String buildColumnDefinition(ImportExcel column) {
        ...
        if (StringUtils.isNotEmpty(column.getRemark())) {
            String remark = SqlValidator.escapeStringLiteral(column.getRemark().replace("\n", " "));
            definition.append(" COMMENT '").append(remark).append("'");
        }
    ...
    public String generateSequenceSql(CreateFrom createFrom, List<ImportExcel> columns) {
        // MySQL使用AUTO_INCREMENT，不需要单独的序列
        return "";
    }
    ...
    private String buildIndexSql(String tableName, IndexDefinition indexDef) {
        ...
        // 索引注释
        if (StringUtils.isNotEmpty(indexDef.getComment())) {
            sb.append(" COMMENT '")
                    .append(SqlValidator.escapeStringLiteral(indexDef.getComment()))
                    .append("'");
        }
